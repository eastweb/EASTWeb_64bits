...
public class ModisLSTV6GlobalDownloader extends GlobalDownloader
{
    ...
    public void run()
    {
        ...
        try {
            ...
            ErrorLog.add(Config.getInstance(), pluginName, metaData.name, "ModisLSTV6GlobalDownloader.run problem while attempting to handle download.", e);
            return;
        }
        ...
        try {
            ...
            ErrorLog.add(Config.getInstance(), pluginName, metaData.name, "ModisLSTV6GlobalDownloader.run problem while setting up current list of missing download files.", e);
        }
        ...
        for(Map.Entry<DataDate, ArrayList<String>> entry : datesFiles.entrySet())
        {
            ...
            try {
                ...
                for (String f : entry.getValue())
                {
                    ...
                    if (f != null)
                    {
                        ModisLSTV6Downloader downloader = new ModisLSTV6Downloader(dd, outFolder, metaData, f);

                        try{
                            downloader.download();
                        } catch (DownloadFailedException e) {
                            ErrorLog.add(Config.getInstance(), pluginName, metaData.name, "ModisLSTV6GlobalDownloader.run problem while running ModisLSTDownloader.", e);
                        } catch (Exception e) {
                            ErrorLog.add(Config.getInstance(), pluginName, metaData.name, "ModisLSTV6GlobalDownloader.run problem while running ModisLSTDownloader.", e);
                        }
                        ...
                        try {
                            ...
                            ErrorLog.add(Config.getInstance(), pluginName, metaData.name, "ModisLSTV6GlobalDownloader.run problem while attempting to add download file.", e);
                        }
                    }
