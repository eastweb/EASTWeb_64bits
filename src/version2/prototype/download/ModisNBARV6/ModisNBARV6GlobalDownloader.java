 ...
 *
 */
public class ModisNBARV6GlobalDownloader extends GlobalDownloader
{
    public ModisNBARV6GlobalDownloader(int myID, Config configInstance, String pluginName, DownloadMetaData metaData, ListDatesFiles listDatesFiles, LocalDate startDate, LocalDate endDate) throws ClassNotFoundException, ParserConfigurationException, SAXException,
    ...
    public void run()
    {
        ...
        try {
            ...
            ErrorLog.add(Config.getInstance(), pluginName, metaData.name, "ModisNBARV6GlobalDownloader.run problem while attempting to handle download.", e);
            return;
        }
        ...
        try {
            ...
            ErrorLog.add(Config.getInstance(), pluginName, metaData.name, "ModisNBARV6GlobalDownloader.run problem while setting up current list of missing download files.", e);
        }
        ...
        for(Map.Entry<DataDate, ArrayList<String>> entry : datesFiles.entrySet())
        {
            ...
            try {
                ...
                for (String f : entry.getValue())
                {
                    ...
                    if (f != null)
                    {
                        ModisNBARV6Downloader downloader = new ModisNBARV6Downloader(dd, outFolder, metaData, f);

                        try{
                            downloader.download();
                        } catch (DownloadFailedException e) {
                            ErrorLog.add(Config.getInstance(), pluginName, metaData.name, "ModisNBARV6GlobalDownloader.run problem while running ModisNBARDownloader.", e);
                        } catch (Exception e) {
                            ErrorLog.add(Config.getInstance(), pluginName, metaData.name, "ModisNBARV6GlobalDownloader.run problem while running ModisNBARDownloader.", e);
                        }
                        ...
                        try {
                            ...
                            ErrorLog.add(Config.getInstance(), pluginName, metaData.name, "ModisNBARV6GlobalDownloader.run problem while attempting to add download file.", e);
                        }
                    }
