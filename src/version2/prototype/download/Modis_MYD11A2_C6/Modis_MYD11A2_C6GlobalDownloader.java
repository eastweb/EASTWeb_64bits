package version2.prototype.download.Modis_MYD11A2_C6;
 ...
 *
 */
public class Modis_MYD11A2_C6GlobalDownloader extends GlobalDownloader
{

    public Modis_MYD11A2_C6GlobalDownloader(int myID, Config configInstance, String pluginName, DownloadMetaData metaData, ListDatesFiles listDatesFiles, LocalDate startDate) throws ClassNotFoundException, ParserConfigurationException, SAXException,
    ...
    public void run()
    {
        ...
        try {
            ...
            ErrorLog.add(Config.getInstance(), pluginName, metaData.name, "Modis_MYD11A2_C6GlobalDownloader.run problem while attempting to handle download.", e);
            return;
        }
        ...
        try {
            ...
            ErrorLog.add(Config.getInstance(), pluginName, metaData.name, "Modis_MYD11A2_C6GlobalDownloader.run problem while setting up current list of missing download files.", e);
        }
        ...
        for(Map.Entry<DataDate, ArrayList<String>> entry : datesFiles.entrySet())
        {
            ...
            try {
                ...
                for (String f : entry.getValue())
                {
                    ...
                    if (f != null)
                    {
                        Modis_MYD11A2_C6Downloader downloader = new Modis_MYD11A2_C6Downloader(dd, outFolder, metaData, f);

                        try{
                            downloader.download();
                        } catch (DownloadFailedException e) {
                            ErrorLog.add(Config.getInstance(), pluginName, metaData.name, "Modis_MYD11A2_C6GlobalDownloader.run problem while running Modis_MYD11A2_C6Downloader.", e);
                        } catch (Exception e) {
                            ErrorLog.add(Config.getInstance(), pluginName, metaData.name, "Modis_MYD11A2_C6GlobalDownloader.run problem while running Modis_MYD11A2_C6Downloader.", e);
                        }
                        ...
                        try {
                            ...
                            ErrorLog.add(Config.getInstance(), pluginName, metaData.name, "Modis_MYD11A2_C6GlobalDownloader.run problem while attempting to add download file.", e);
                        }
                    }
